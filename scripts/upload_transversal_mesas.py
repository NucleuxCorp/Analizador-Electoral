...
Upload lab transversal JPEGs to Supabase Storage (public CDN).

Object layout:
  {bucket}/{E14C|D|T_conflictivas}/{mesa_key}/{e14c|e14d|e14t}_pNN.jpg
...
def collect_files(mesas_dir: Path, dataset: str) -> list[tuple[Path, str]]:
    """Return (local_path, storage_object_key) for every JPEG."""
    prefix = storage_object_prefix(dataset)
    out: list[tuple[Path, str]] = []
    for mesa_dir in sorted(p for p in mesas_dir.iterdir() if p.is_dir()):
        mk = mesa_dir.name
        for jpg in sorted(mesa_dir.glob("*.jpg")):
            out.append((jpg, f"{prefix}/{mk}/{jpg.name}"))
    return out
