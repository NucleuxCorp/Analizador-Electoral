...
Subcommands:
  ...
  prepare-gallery   Convert PDFs to WebP page images + manifest.json (after reprocess)
...
_LAB_ROOT = Path(r"E:\Nucleux\tools\Analizador de Elecciones\Laboratorio\analisis_transversal")
SHARED_MESAS_DIR = _LAB_ROOT / "mesas_compartidas"
...
def _checkpoint_path() -> Path:
    """Dynamic checkpoint path — LAB_DIR is only set after _configure_dataset()."""
    return LAB_DIR / "reprocess.checkpoint.json"
...
def cmd_revalidate(args: argparse.Namespace) -> None:
    ...
    if args.reprocess:
        ...
        # Write output BEFORE checkpoint so a checkpoint failure never destroys results
        try:
            _save_checkpoint(done_keys)
        except Exception as exc:
            print(f"[revalidate] WARNING: checkpoint save failed: {exc}")
    else:
        results = []
        for row in index_rows:
            audit = field_audit({
...
def cmd_prepare_gallery(args: argparse.Namespace) -> None:
    ...
    # Pre-filter: separate mesas that already have images from those that need work.
    # This makes tqdm's it/s reflect ONLY real work (converted), not already_done skips.
    already_existing = set()
    if mesas_dir.exists():
        already_existing = {p.name for p in mesas_dir.iterdir() if p.is_dir()}
    rows_to_process = []
    pre_skipped = 0

    with tqdm(total=len(rows), desc="pre-scan", unit="mesa") as scan_pbar:
        for row in rows:
            mk = f"{row['dept']}_{row['mpio']}_{row['zona']}_{row['puesto']}_{row['mesa']}"
            if mk in already_existing:
                pre_skipped += 1
            else:
                rows_to_process.append(row)
            scan_pbar.update(1)
            scan_pbar.set_postfix_str(f"skip={pre_skipped} todo={len(rows_to_process)}")

    print(f"[prepare-gallery] pre-skip: {pre_skipped:,} mesas already have images")
    print(f"[prepare-gallery] to process: {len(rows_to_process):,} mesas (real work)")

    def _classify(r: dict) -> str:
        nonlocal processed_count, skipped_count
        did_work = False
        for src in ("e14t", "e14d", "e14c"):
            status = r["sources"].get(src, "")
            if status == "missing":
                missing_count[src] += 1
            elif status.startswith("already_done"):
                pass
    ...
    work_total = len(rows_to_process)
    if args.workers > 1:
        with ThreadPoolExecutor(max_workers=args.workers) as ex:
            futs = [ex.submit(do_mesa, row) for row in rows_to_process]
            with tqdm(total=work_total, desc="gallery") as pbar:
                for fut in as_completed(futs):
                    r = fut.result()
                    _classify(r)
                    pbar.update(1)
                    pbar.set_postfix_str(
                        f"proc={processed_count} skip={skipped_count} miss={sum(missing_count.values())}"
                    )
    else:
        with tqdm(total=work_total, desc="gallery") as pbar:
    ...
    print(f"[prepare-gallery] processed: {processed_count:,} (generated images)")
    print(f"[prepare-gallery] skipped:   {skipped_count:,} (already_done during run)")
    print(f"[prepare-gallery] pre-skip:  {pre_skipped:,} (already had folder, never submitted)")
    print(f"[prepare-gallery] elapsed:   {elapsed:.1f}s")
    if processed_count > 0:
        real_rate = processed_count / elapsed if elapsed > 0 else 0
        print(f"[prepare-gallery] real speed: {real_rate:.2f} mesas/s (processed only)")
    if skipped_count > 0 and total > 0:
        apparent_rate = total / elapsed if elapsed > 0 else 0
        print(f"[prepare-gallery] apparent speed: {apparent_rate:.2f} mesas/s (incl. skips)")
...
def main() -> None:
    ...
    p_gal = sub.add_parser("prepare-gallery", help="Build manifest + WebP page images from index")
