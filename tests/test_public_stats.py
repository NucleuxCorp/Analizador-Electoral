...
# ---------------------------------------------------------------------------
# Phase 1.7 — test_api_route_200_json_shape
# ---------------------------------------------------------------------------

class TestApiPublicStatsRoute:
    """GET /api/public-stats returns 200 JSON with correct shape."""

    def test_route_returns_200_json(self, prod_app):
        """Normal request: HTTP 200 with application/json content type."""
        known = {
            "mesas_all_three": 500,
            "mesas_analyzed": 1000,
    ...
    def test_route_body_values_match_db(self, prod_app):
        """Response body values match what get_public_stats() returned."""
        known = {
            "mesas_all_three": 500,
            "mesas_analyzed": 1000,
            "mesas_remaining": 121_020,
...
# ---------------------------------------------------------------------------
# Phase 1.8 — test_api_route_no_auth_required
# ---------------------------------------------------------------------------

class TestApiPublicStatsNoAuth:
    """GET /api/public-stats is accessible without authentication."""

    def test_no_auth_returns_200(self, prod_app):
        """No session cookie → HTTP 200, not 401 or 403."""
        with patch("src.modules.labeler.db.get_public_stats", return_value=ZEROS_DICT):
            client = prod_app.test_client()
            resp = client.get("/api/public-stats")
...
# ---------------------------------------------------------------------------
# Phase 1.9 — test_api_route_db_raises_returns_zeros
# ---------------------------------------------------------------------------

class TestApiPublicStatsDbRaises:
    """When get_public_stats() raises, route returns 200 with zeros."""

    def test_db_exception_returns_200_with_zeros(self, prod_app):
        """Exception in db layer → route still returns 200 with zeros dict."""
        with patch("src.modules.labeler.db.get_public_stats", side_effect=RuntimeError("db down")):
            client = prod_app.test_client()
            resp = client.get("/api/public-stats")

        assert resp.status_code == 200
        data = resp.get_json()
        assert data["total_universe"] == 122_020
        assert data["mesas_all_three"] == 0
