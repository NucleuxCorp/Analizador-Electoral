...
accessors (T7 + Phase 0 + transversal-decisions-port).
...
class _LiveDbModule:
    """Resolves `src.modules.labeler.db` fresh from sys.modules on every
    attribute access, instead of binding once at collection time.

    Some test suites in this repo (e.g. tests/labeler/test_security_audit.py's
    TestPhase2_PrivilegeEscalation fixture) temporarily swap
    sys.modules["src.modules.labeler.db"] for a mock and later `.pop()` it,
    which forces the NEXT import to re-execute db.py and create a brand-new
    module object. A plain `import src.modules.labeler.db as db` at file
    scope binds to whichever object existed at collection time — if that
    swap/pop happens (from another test file, in full-suite runs) before
    this file's tests execute, `db.xxx(...)` silently calls the stale old
    module while `unittest.mock.patch("src.modules.labeler.db._client", ...)`
    (which re-resolves via sys.modules at patch time) patches the new one,
    so the patch has no effect and real (unmocked) code runs instead.
    """

    def __getattr__(self, name):
        return getattr(sys.modules["src.modules.labeler.db"], name)


import src.modules.labeler.db  # noqa: F401 — ensure it's imported at least once
db = _LiveDbModule()
...
class TestPortedConstantsAndImport:
    ...
    def test_transversal_fields_not_imported_from_alerts(self):
        ...
        source = inspect.getsource(sys.modules["src.modules.labeler.db"])
